...
const RecibosSystem = {
    ...
    init() {
        if (!window.bootstrap || !window.Chart || !window.jspdf || !window.saveAs) {
            console.error('Dependencias faltantes: Bootstrap, Chart.js, jsPDF o FileSaver.js');
    ...
    loadData() {
        try {
            const storedData = localStorage.getItem('recibosData');
            this.data = storedData ? JSON.parse(storedData) : { luz: {}, agua: {}, config: { floors: ['1', '2', '3'], username: '', password: '', phone: '' } };
            if (!this.data.luz || !this.data.agua || !this.data.config || !Array.isArray(this.data.config.floors)) {
                throw new Error('Datos corruptos');
            }
            this.floors = this.data.config.floors;
        } catch (e) {
            console.error('Error al cargar datos:', e);
            this.showAlert('Error al cargar datos. Se inicializarán valores por defecto.', 'warning', null);
    ...
    saveData() {
        try {
            localStorage.setItem('recibosData', JSON.stringify(this.data));
            this.updateDashboard();
        } catch (e) {
            console.error('Error al guardar datos:', e);
            this.showAlert('Error al guardar datos en localStorage. Es posible que esté lleno.', 'danger', null);
        }
    },
    ...
    calculate(service, event) {
        ...
        let consumption, subTotal, impuesto, total, resultHTML;
        ...
        if (service === 'agua') {
           ...
           // if (consumption < 0) {
              //  this.showAlert('El consumo actual debe ser mayor o igual al consumo anterior.', 'danger', service);
               // return;
           // }
            ...
            this.data[service][floor][year][month] = {
    ...
    saveEditedReceipt(event) {
        ...
        if (inputs.service === 'agua') {
            ...
            if (consumption < 0) {
                this.showAlert('El consumo actual debe ser mayor o igual al consumo anterior.', 'danger', 'report');
                return;
            }
            ...
            this.data[inputs.service][inputs.floor][inputs.year][inputs.month] = {
                prev: inputs.prev,
                curr: inputs.curr,
                consumption,
                price: inputs.price,
    ...
    deleteReceipt(service, floor, year, month) {
